...
try {
  ...
  if (
    ...
    throw new Error(
      "PWRC_CLIENT_API_URL_CREDENTIALS_FORBIDDEN",
    );
  }
    ...
    if (
      ...
      for await (
        ...
        if (
          ...
          response.end(
            '{"ok":false,"error":{"code":"PWRC_CLIENT_PROXY_BODY_TOO_LARGE"}}\n',
          );
          return;
        }
      ...
      JSON.stringify({
        ...
        error: {
          code:
            error?.name ===
              "AbortError"
              ? "PWRC_CLIENT_PROXY_TIMEOUT"
              : "PWRC_CLIENT_PROXY_UNAVAILABLE",
        },
      }) + "\n",
    );
  } finally {
...
server.on(
  ...
  (error) => {
    ...
    if (code === "EADDRINUSE") {
      console.error(
        `PWRC_SERVER_LISTEN_ERROR:client:EADDRINUSE:${host}:${port}`,
      );
    }
