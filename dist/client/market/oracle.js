...
import { assertFreshObservation, assertProviderAgreement, assertTradeLiquidity, } from "../../src/market/risk.js";
export async function getVerifiedPwrcMarketSnapshot(input) {
    let pythObservation;
    let birdeyeObservation;
    let liquidityUsd;
    ...
    if (input.requireLiquidity) {
        if (liquidityUsd === undefined)
            throw new Error("PWRC_MARKET_LIQUIDITY_UNAVAILABLE");
        assertTradeLiquidity(liquidityUsd);
    }
    const primary = pythObservation ?? birdeyeObservation;
    if (!primary)
        throw new Error("PWRC_VERIFIED_MARKET_PRICE_UNAVAILABLE");
    return {
