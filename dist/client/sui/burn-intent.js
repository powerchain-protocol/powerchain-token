...
export function buildPauseWpwrcBridgeTransaction(input) {
    ...
    tx.moveCall({
        target: `${input.deployment.packageId}::bridge::set_paused`,
        arguments: [
 ...
 */
export function buildStageCanonicalBurnIntentTransaction(input) {
    if (input.quarterId < 20271n) {
        throw new Error("WPWRC_BURN_INTENT_QUARTER_INVALID");
    }
    if (input.expectedPostBurnWrappedCeilingBaseUnits < 0n) {
        throw new Error("WPWRC_BURN_INTENT_CEILING_INVALID");
    }
    assertDigest32(input.planSha256);
    const tx = new Transaction();
    tx.moveCall({
        target: `${input.deployment.packageId}::bridge::stage_canonical_burn_intent`,
        arguments: [
            tx.object(input.deployment.bridgeControllerId),
            tx.pure.u64(input.quarterId),
            tx.pure.u64(input.expectedPostBurnWrappedCeilingBaseUnits),
            tx.pure.vector("u8", [...input.planSha256]),
        ],
    });
    return tx;
}
export function buildCancelCanonicalBurnIntentTransaction(input) {
    const tx = new Transaction();
    tx.moveCall({
        target: `${input.deployment.packageId}::bridge::cancel_canonical_burn_intent`,
        arguments: [
            tx.object(input.deployment.bridgeControllerId),
        ],
    });
...
//# sourceMappingURL=burn-intent.js.map