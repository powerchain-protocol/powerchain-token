export function evaluateBridgeConservation(input) {
    const pendingToSui = input.pendingSolanaToSuiBaseUnits ?? 0n;
    const pendingToSolana = input.pendingSuiToSolanaBaseUnits ?? 0n;
    const errors = [];
    const exposure = input.wrappedSupplyBaseUnits +
        pendingToSui +
        pendingToSolana;
    ...
    if (exposure > input.canonicalLiveSupplyBaseUnits) {
        errors.push("WRAPPED_EXPOSURE_EXCEEDS_LIVE_CANONICAL_SUPPLY");
    }
    return {
        valid: errors.length === 0,
        effectiveWrappedExposureBaseUnits: exposure,
        backingSurplusBaseUnits: exposure >= 0n &&
