...
export function quoteQuarterlyBurnFromLiveSupply(currentCanonicalSupplyBaseUnits) {
    if (currentCanonicalSupplyBaseUnits <= 0n ||
        currentCanonicalSupplyBaseUnits > PWRC_MAX_BASE_UNITS) {
        throw new Error("PWRC_CANONICAL_SUPPLY_INVALID");
    }
    const targetBurnBaseUnits = currentCanonicalSupplyBaseUnits *
    ...
    return {
        ...
        targetBurnBaseUnits,
        postBurnCanonicalSupplyBaseUnits: currentCanonicalSupplyBaseUnits - targetBurnBaseUnits,
    };
}
...
export function effectiveWrappedExposure(input) {
    const exposure = input.suiWrappedSupplyBaseUnits +
