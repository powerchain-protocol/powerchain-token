export const MAX_READ_RETRY_ATTEMPTS = 10;
export const MAX_READ_RETRY_DELAY_MS = 60_000;
...
export function retryDelayMs(attempt, policy) {
    ...
    const candidate = policy.baseDelayMs *
        2 ** exponent;
    if (!Number.isSafeInteger(candidate)) {
        return policy.maxDelayMs;
    }
    return Math.min(candidate, policy.maxDelayMs);
}
...
export async function withReadRetry(operation, options = {}) {
    ...
    throw (lastError ??
        new Error("POWERCHAIN_RETRY_EXHAUSTED"));
}
