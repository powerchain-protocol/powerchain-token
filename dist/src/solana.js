...
export const POWERCHAIN_FORBIDDEN_DEPLOYMENT_PROGRAM_IDS = new Set([
...
export function assertPowerChainDeploymentProgramId(value, cluster) {
    let key;
    try {
    ...
    if (POWERCHAIN_FORBIDDEN_DEPLOYMENT_PROGRAM_IDS.has(normalized)) {
        throw new Error(normalized === SOLANA_SYSTEM_PROGRAM_ID
...
export function resolvePowerChainProgramId(kind, cluster, explicit, env = process.env) {
    const local = kind === "pwrc-lock"
