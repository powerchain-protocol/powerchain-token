...
`src/common/` owns TypeScript runtime behavior. `src/utils/` is a stable public
re-export layer and does not reimplement the same logic.

Root `utils/` owns dependency-free Node utilities used by `.mjs` build/release
scripts:
...
not diverge from `utils/atomic-json.mjs`.
...
The dependency-free root `utils/` layer also owns cross-script platform
services:
...
`utils/process.mjs` always invokes child commands with `shell: false`, bounded
timeouts, bounded output buffers, and explicit failure policy.

`utils/logger.mjs` emits structured JSON and recursively redacts fields with
...
`apps/api` and `apps/web` are workspace applications. They consume the
...
`apps/web` uses a same-origin reverse proxy for `/api/*`. This keeps server-only
