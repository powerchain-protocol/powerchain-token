...
Reusable utilities are under `src/common/` for URL validation, environment parsing, bounded retry, timeouts, bigint/u64 validation and domain-separated hashing. Read handlers may retry bounded transient failures; write handlers never blind-retry and require reconciliation.
