...
Canonical PWRC uses Token-2022 with 9 decimals, fixed genesis/max supply, revoked mint authority after finalization, null freeze authority, and only `MetadataPointer` + `TokenMetadata` extensions. `TransferFeeConfig` is required and must match the canonical 250 bps / 1,000,000 PWRC cap.
...
## Transfer-fee authority evidence

PWRC uses Token-2022 `TransferFeeConfig`. Production verification records both
