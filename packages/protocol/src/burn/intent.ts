...
export function assertQuarterlyBurnIntent(
  intent: QuarterlyBurnIntent,
): void {
  ...
  const pre = BigInt(
    intent.preBurnCanonicalSupplyBaseUnits,
  );
  const target = BigInt(
    intent.targetBurnCanonicalBaseUnits,
  );
  const post = BigInt(
    intent.expectedPostBurnCanonicalSupplyBaseUnits,
  );
  const wrappedCeiling = BigInt(
    intent.expectedPostBurnWrappedCeilingBaseUnits,
  );
  ...
  if (wrappedCeiling !== expectedWrappedCeiling) {
    throw new Error(
      "PWRC_BURN_INTENT_WRAPPED_CEILING_MISMATCH",
    );
  }
...
export function quarterlyBurnIntentSha256(
  intent: QuarterlyBurnIntent,
): string {
