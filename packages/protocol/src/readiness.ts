...
export function evaluateProductionReadiness(
  ...
  return [
    ...
    },
    {
      id: "cctp",
      state:
        input.cctpMessageTransmitter && input.cctpTokenMessengerMinter
          ? "READY"
          : "OPTIONAL",
      reason:
        input.cctpMessageTransmitter && input.cctpTokenMessengerMinter
...
export function assertCoreProductionReady(checks: readonly ReadinessCheck[]): void {
  const blocked = checks.filter((x) => x.state === "BLOCKED");
  if (blocked.length) {
    throw new Error(
      `PWRC_PRODUCTION_NOT_READY:${blocked.map((x) => x.id).join(",")}`,
    );
  }
}
