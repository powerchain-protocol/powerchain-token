...
node scripts/verify-chain.mjs "$PWRC_MINT" genesis > "$OUT/evidence/pre-finalization.json"
...
node scripts/verify-chain.mjs "$PWRC_MINT" finalized | tee "$OUT/evidence/final-verification.json"
