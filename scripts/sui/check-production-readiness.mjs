...
if (!["testnet", "mainnet"].includes(network)) {
  throw new Error("Usage: node check-production-readiness.mjs <testnet|mainnet>");
}

const cfg = JSON.parse(fs.readFileSync("config/sui/wpwrc.json", "utf8"));
const n = cfg.networks[network];
const checks = [];
const check = (id, ok, reason) =>
  checks.push({ id, state: ok ? "READY" : "BLOCKED", reason });
...
check(
  "role-separation",
  Boolean(operator && governor && operator !== governor),
  operator && governor && operator !== governor
    ? "Operator and governor are separate"
    : "Distinct operator and governor required",
);

if (network === "mainnet") {
  const moveLock = "contracts/wpwrc/Move.lock";
  const moveLockPresent = fs.existsSync(moveLock);
  const moveLockSha256 = moveLockPresent
    ? crypto.createHash("sha256").update(fs.readFileSync(moveLock)).digest("hex")
    : null;
  check("move-lock", moveLockPresent, moveLockPresent ? `Move.lock ${moveLockSha256}` : "Move.lock missing; run reviewed Sui build");
...
const result = {
  version: "1.0.0",
  network,
  ready: checks.every((x) => x.state === "READY"),
  checks,
};
fs.mkdirSync("reports", { recursive: true });
fs.writeFileSync(
  `reports/wpwrc-${network}-readiness.json`,
