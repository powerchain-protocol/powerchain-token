...
export function assertBridgeDeploymentManifest(
  ...
  if (manifest.canonicalDecimals !== 9) {
    throw new Error("PWRC_BRIDGE_MANIFEST_CANONICAL_DECIMALS_INVALID");
  }
  if (manifest.wrappedDecimals !== 9) {
    throw new Error("PWRC_BRIDGE_MANIFEST_WRAPPED_DECIMALS_INVALID");
  }
  if (manifest.baseUnitFactor !== "1") {
    throw new Error("PWRC_BRIDGE_MANIFEST_FACTOR_INVALID");
  }
  if (manifest.wrappedGenesisSupplyBaseUnits !== "0") {
    throw new Error("PWRC_BRIDGE_MANIFEST_GENESIS_INVALID");
  }
  if (manifest.wrappedMintPolicy !== "bridge-only") {
