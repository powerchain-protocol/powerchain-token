...
export const PWRC_BURN_START_YEAR = 2027 as const;
export const PWRC_BURN_START_QUARTER = 1 as const;
export const PWRC_BURN_START_QUARTER_ID = 20271n as const;

/**
 * Canonical quarter ID encoding:
 * YYYYQ, e.g. 2027 Q1 => 20271.
 */
export function encodeQuarterId(year: number, quarter: Quarter): bigint {
...
export function decodeQuarterId(id: bigint): {
  year: number;
  quarter: Quarter;
} {
  if (id <= 0n || id > 99_999n) {
  ...
  const raw = Number(id);
  const quarter = (raw % 10) as Quarter;
...
export function assertQuarterAtOrAfterBurnStart(id: bigint): void {
  const current = decodeQuarterId(id);
  const start = {
    year: PWRC_BURN_START_YEAR,
    quarter: PWRC_BURN_START_QUARTER as Quarter,
  };
...
export function nextQuarterId(id: bigint): bigint {
  const { year, quarter } = decodeQuarterId(id);
  if (quarter === 4) return encodeQuarterId(year + 1, 1);
  return encodeQuarterId(year, (quarter + 1) as Quarter);
}

export function burnSequenceForQuarter(id: bigint): bigint {
  assertQuarterAtOrAfterBurnStart(id);

  const { year, quarter } = decodeQuarterId(id);
  const elapsedQuarters =
    BigInt((year - PWRC_BURN_START_YEAR) * 4) +
    BigInt(quarter - PWRC_BURN_START_QUARTER);

  return elapsedQuarters + 1n;
}

export function assertContiguousQuarter(
