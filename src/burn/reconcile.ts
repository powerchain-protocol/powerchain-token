...
export function reconcileQuarterlyBurn(
  input: QuarterlyBurnReconciliationInput,
): void {
  if (
    input.observedSolanaSupplyBaseUnits !==
  ...
  if (
    input.observedSuiCanonicalCeilingBaseUnits !==
    input.expectedPostBurnSupplyBaseUnits
  ) {
  ...
  const effectiveExposure =
    input.observedSuiWrappedSupplyBaseUnits +
