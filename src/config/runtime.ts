export const POWERCHAIN_RUNTIME_VERSION =
  "1.0.0" as const;
...
export function assertRuntimePolicy(
  ...
  if (
    policy.blindWriteRetries !==
    false
  ) {
    throw new Error(
      "PWRC_BLIND_WRITE_RETRIES_FORBIDDEN",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "PWRC_FINALITY_MUST_BE_FINALIZED",
    );
  }

  if (
    policy.preflightCommitment !==
    "confirmed"
  ) {
    throw new Error(
      "PWRC_PREFLIGHT_COMMITMENT_INVALID",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "PWRC_RUNTIME_RETRY_ATTEMPTS_INVALID",
    );
  }
